export const userData4 = { 
  featureInfo : [
    { title: "Total Applications", data: 200 },
    { title: "Total Disbursed", data: 5000 },
    { title: "Total Rejected", data: 700 }
  ], 
  charts : [
    { title: "Pending Documentation", data: 600, type: "Bar", color:"#78efff",
        chartData: [
          { name: 'Jan', "Active User": 2000},
          { name: 'Feb', "Active User": 6000},
          { name: 'Mar', "Active User": 1200},
          { name: 'Apr', "Active User": 1000},
          { name: 'May', "Active User": 4300},
          { name: 'Jun', "Active User": 4900},
          { name: 'Jul', "Active User": 7700},
          { name: 'Aug', "Active User": 9000},
          { name: 'Sep', "Active User": 2300},
          { name: 'Oct', "Active User": 3300},
          { name: 'Nov', "Active User": 4500},
          { name: 'Dec', "Active User": 8900}
        ]
    }, 
    { title: "Pending Offer Acceptance", data: 87, type: "Area", color:"#fc7d5d",
        chartData: [
          { name: 'Jan', "Active User": 2000},
          { name: 'Feb', "Active User": 6000},
          { name: 'Mar', "Active User": 1200},
          { name: 'Apr', "Active User": 1000},
          { name: 'May', "Active User": 4300},
          { name: 'Jun', "Active User": 4900},
          { name: 'Jul', "Active User": 7700},
          { name: 'Aug', "Active User": 9000},
          { name: 'Sep', "Active User": 2300},
          { name: 'Oct', "Active User": 3300},
          { name: 'Nov', "Active User": 4500},
          { name: 'Dec', "Active User": 8900}
        ]
    },
    { title: "Pending Disbursment", data: 123, type: "Bar", color:"#4c75fc",
        chartData: [
          { name: 'Jan', "Active User": 2000},
          { name: 'Feb', "Active User": 6000},
          { name: 'Mar', "Active User": 1200},
          { name: 'Apr', "Active User": 1000},
          { name: 'May', "Active User": 4300},
          { name: 'Jun', "Active User": 4900},
          { name: 'Jul', "Active User": 7700},
          { name: 'Aug', "Active User": 9000},
          { name: 'Sep', "Active User": 2300},
          { name: 'Oct', "Active User": 3300},
          { name: 'Nov', "Active User": 4500},
          { name: 'Dec', "Active User": 8900}
        ]
    },
    { title: "Failed Transaction", data: 78, type: "Area", color:"#b326ff",
        chartData: [
          { name: 'Jan', "Active User": 2000},
          { name: 'Feb', "Active User": 6000},
          { name: 'Mar', "Active User": 1200},
          { name: 'Apr', "Active User": 1000},
          { name: 'May', "Active User": 4300},
          { name: 'Jun', "Active User": 4900},
          { name: 'Jul', "Active User": 7700},
          { name: 'Aug', "Active User": 9000},
          { name: 'Sep', "Active User": 2300},
          { name: 'Oct', "Active User": 3300},
          { name: 'Nov', "Active User": 4500},
          { name: 'Dec', "Active User": 8900}
        ]
    }
  ],
  pieChart: { title: "LSP Disbursals / Lender", data: 2.5,
             chartData: [
              { name: 'Amazon', "Active User": 5000},
              { name: 'Sanpdeal', "Active User": 300},
              { name: 'GeM', "Active User": 3000}
            ] 
  }
,
  Table : [
    { lsp: "Snapdeal", l_type: "Invoice Financing", Amt: "$10,000", DT: "dd/mm/yy - hh:mm:ss", Status: "Approved" },
    { lsp: "Snapdeal", l_type: "Invoice Financing", Amt: "$10,000", DT: "dd/mm/yy - hh:mm:ss", Status: "Approved" },
    { lsp: "Snapdeal", l_type: "Invoice Financing", Amt: "$10,000", DT: "dd/mm/yy - hh:mm:ss", Status: "Approved" },
    { lsp: "Snapdeal", l_type: "Invoice Financing", Amt: "$10,000", DT: "dd/mm/yy - hh:mm:ss", Status: "Approved" },
    { lsp: "Snapdeal", l_type: "Invoice Financing", Amt: "$10,000", DT: "dd/mm/yy - hh:mm:ss", Status: "Approved" },
    { lsp: "Snapdeal", l_type: "Invoice Financing", Amt: "$10,000", DT: "dd/mm/yy - hh:mm:ss", Status: "Approved" },
    { lsp: "Snapdeal", l_type: "Invoice Financing", Amt: "$10,000", DT: "dd/mm/yy - hh:mm:ss", Status: "Approved" },
    { lsp: "Snapdeal", l_type: "Invoice Financing", Amt: "$10,000", DT: "dd/mm/yy - hh:mm:ss", Status: "Approved" },
    { lsp: "Snapdeal", l_type: "Invoice Financing", Amt: "$10,000", DT: "dd/mm/yy - hh:mm:ss", Status: "Approved" },
    { lsp: "Snapdeal", l_type: "Invoice Financing", Amt: "$10,000", DT: "dd/mm/yy - hh:mm:ss", Status: "Approved" }
  ]
};